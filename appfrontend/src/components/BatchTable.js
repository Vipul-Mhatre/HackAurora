...
export default class BatchTable extends React.Component {
    ...
    handleSubmit = () => {
        ...
        const Ldata = existingData ? JSON.parse(existingData) : []; // Retrieve and parse existing data from local storage
        console.log(Ldata);
                        ...
                        <TableBody>
                            {this.props.rows.length > 0 ? (
                                this.props.rows
                                    ...
                                    .map((row) => {
                                        // Check if data exists for the current productId in Ldata
