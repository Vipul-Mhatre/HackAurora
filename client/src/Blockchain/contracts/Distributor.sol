// SPDX-License-Identifier: MIT
pragma solidity >=0.4.21 <0.9.0;

import "./Users.sol";

/**
 * @title Distributor
 * @dev Handles distributor functions.
 */
contract Distributor {
    
    using Users for Users.User;
    Users.User private distributors;

    function addDistributor(address newDistributor, string memory name) public {
        require(!distributors.isExistingUser(newDistributor), "Distributor with this address already exists!");
        distributors.addUser(newDistributor, name);
    }

    function isDistributor() public view returns (bool) {
        return distributors.isExistingUser(msg.sender);
    }

    function getDistributorName(address distributorAddress) public view returns (string memory) {
        return distributors.getUserName(distributorAddress);
    }
}
