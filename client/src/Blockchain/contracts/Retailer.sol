...
contract Retailer {
    ...
    function addRetailer(address newRetailer, string memory name) public {
        require(!retailers.isExistingUser(newRetailer), "Retailer with this address already exists!");
        retailers.addUser(newRetailer, name);
    }
    ...
    function getRetailerName(address retailerAddress) public view returns (string memory) {
        return retailers.getUserName(retailerAddress);
    }
}
